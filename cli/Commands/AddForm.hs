...
createController ctrlPath modPrefix actionName honeypotField minTime useFlash redirectPage = do
    ...
    let content = T.pack $ unlines
            ...
            , ""
            , "-- TODO: Send email (fill .env with SMTP vars, see env.example)"
            , "-- loadSmtpConfig :: Action (Maybe SmtpConfig)"
            , "-- loadSmtpConfig = do"
            , "--     mHost <- lookupEnv \"SMTP_HOST\""
            , "--     mPort <- lookupEnv \"SMTP_PORT\""
            , "--     mUser <- lookupEnv \"SMTP_USERNAME\""
            , "--     mPass <- lookupEnv \"SMTP_PASSWORD\""
            , "--     pure $ SmtpConfig"
            , "--         <$> mHost"
            , "--         <*> (readMaybe =<< mPort)"
            , "--         <*> mUser"
            ...
            , "            , minSubmitTime " ++ minTime ++ "  -- remove if not using the guard minSubmitTime"
            , "            , mxRecord \"email\""
            , "            ]"
            , "        )"
            , ""
